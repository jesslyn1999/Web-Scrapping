...
class UrlExtractor(Spider):
    ...
    def __init__(self, root=None, depth=0, *args, **kwargs):
        ...
        if self.options.get("allow_domains") is None:
            ...
            for root_url in self.start_urls:
                ...
                self.options["allow_domains"].add(
        ...
        self.le = LinkExtractor(allow=self.options.get("allow"), deny=self.options.get("deny"),
                                allow_domains=self.options.get(
                                    "allow_domains"),
                                deny_domains=self.options.get(
                                    "deny_domains"),
                                restrict_xpaths=self.options.get(
                                    "restrict_xpaths"),
                                canonicalize=False,
                                unique=True, process_value=None, deny_extensions=None,
                                restrict_css=self.options.get(
                                    "restrict_css"),
                                strip=True)
    ...
    def parse_req(self, response):
        ...
        if not response.meta["parent_url"] in self.result:
            self.result[response.meta["parent_url"]] = []
        if item["Body"]:
            self.result[response.meta["parent_url"]].append(item)
    ...
    def clean_options(self):
        allowed_options = ["allow", "deny", "allow_domains",
...
def create_crawler_class():
    return UrlExtractor, UrlExtractor.result
